...
void send_to_thingspeak(float temperature, float humidity) {
  ...
  snprintf(payload, sizeof(payload), "field1=%.1f&field2=%.1f", temperature,
           humidity);
...
void esp_task(void *arg) {
  ...
  while (1) {
    ...
    // Read sensors
    int adc_raw = mq135_read(adc_handle);
    float ppm = mq135_adc_to_ppm(adc_raw);
    if (ppm <= 300) {
      snprintf(dataAQ, sizeof(dataAQ), " AQ: Good");

    } else if (ppm <= 700) {
      snprintf(dataAQ, sizeof(dataAQ), " AQ: Moderate");

    } else {
      snprintf(dataAQ, sizeof(dataAQ), " AQ: Bad");
    }
